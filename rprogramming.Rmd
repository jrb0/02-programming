...
Note that I use generic object and element names wherever possible. For example, `summary(my_data$var1)` refers to a generic object, `mydata` with a column named `var1`.
...
## Function basics
R packages contain a library of functions. A function is a pre-defined routine to perform some task. For example, the command `count` calls a `dplyr` function that tallies the unique values that appear in one or more variables.

Functions have three main parts: 
- Name: how to call the function (e.g., `count` or `summary`).  
...
Notice in comparing these sequences where the "piped" output fits in the next code segment. For example, `count(x = mydata, var1)` becomes `mydata %>% count(x = ., var1)` where `mydata` flows directly into `.`, which serves here as an explicit placeholder for the input from a prior operation. You can simplify to `mydata %>% count(var1)`.
